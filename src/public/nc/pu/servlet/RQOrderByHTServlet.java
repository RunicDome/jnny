...
// 对接二维码，燃气采购订单JSON（关联采购合同）导入
@SuppressWarnings({ "restriction", "unused", "unchecked", "rawtypes" })
public class RQOrderByHTServlet extends HttpServlet implements
		IHttpServletAdaptor {
	...
	public void doAction(HttpServletRequest req, HttpServletResponse res)
		...
		Debug.error("=============rqm21upload order msg ：" + body);
	...
	private String createVO(String body) {
		...
		try {
			...
			for (int k = 0; k < paramList.size(); k++) {
				...
				String pk_ct_pu = headInfo.get("pk_ct_pu") + "";// 采购合同主键
				AggCtPuVO ctAggvo = null; // 采购合同VO
				orderVO = new OrderVO();
				// 获取采购合同
				try {
					List<Map<String, Object>> htbodyInfo = (List) map
							.get("bill_b");
					String[] bpks = new String[htbodyInfo.size()];// 采购合同明细主键
					for (int i = 0; i < htbodyInfo.size(); i++) {
						bpks[i] = htbodyInfo.get(i).get("pk_ct_pu_b") + "";
					}
					ctAggvo = getCtAggVO(pk_ct_pu, bpks);
					// 调用方法将采购合同自动转采购订单
					IPfExchangeService pf = NCLocator.getInstance().lookup(
							IPfExchangeService.class);
					orderVO = (OrderVO) pf.runChangeData("Z2", "21-Cxx-RD-96",
							ctAggvo, null);
				} catch (BusinessException e) {
					// TODO: handle exception
					return createMsg(false, "-1", e.getMessage());
				}
				OrderHeaderVO hVO = new OrderHeaderVO();
				if (ctAggvo != null && pk_ct_pu != null) {
					hVO = orderVO.getHVO();
				...
				String pk_vdef9 = (String) hypubBo.findColValue("BD_DEFDOC",
						"PK_DEFDOC",
				...
				if (StringUtils.isEmpty(pk_purchaseorg)) {
				...
				String pk_dept = (String) hypubBo.findColValue("org_dept",
						"pk_dept", "nvl(dr,0) = 0 and code = '" + dept + "'  and pk_org " +
								"= '"+pk_org+"'");
				if (isEmpty(pk_dept)) {
	...
	private AggCtPuVO getCtAggVO(String pk_ct_pu, String[] bpks)
			throws BusinessException {
		if (pk_ct_pu == null || "null".equals(pk_ct_pu)) {
			throw new BusinessException("请传入合同主键！");
		}
		if (bpks == null || bpks.length <= 0) {
			throw new BusinessException("请传入合同明细主键！");
		}
		AggCtPuVO aggvo = new AggCtPuVO();
		CtPuVO hvo = (CtPuVO) hypubBo.queryByPrimaryKey(CtPuVO.class, pk_ct_pu);
		if (hvo == null) {
			throw new BusinessException("根据采购合同主键[" + pk_ct_pu + "]获取采购合同失败！");
		}
		aggvo.setParent(hvo);
		CtPuBVO[] bvos = new CtPuBVO[bpks.length];
		for (int i = 0; i < bpks.length; i++) {
			if (bpks[i] == null || "null".equals(bpks[i])) {

			}
			CtPuBVO[] bvoi = (CtPuBVO[]) hypubBo.queryByCondition(
					CtPuBVO.class, "nvl(dr,0) = 0 and pk_ct_pu_b = '" + bpks[i]
							+ "'");
			if (bvoi == null || bvoi.length <= 0) {
				throw new BusinessException("根据采购合同明细主键[" + bpks[i]
						+ "]获取采购合同明细失败！");
			}
			if (!bvoi[0].getPk_ct_pu().equals(pk_ct_pu)) {
				throw new BusinessException("采购合同明细主键[" + bpks[i]
						+ "]对应采购合同主键[" + pk_ct_pu + "]失败，请检查是否为同一合同！");
			}
			bvos[i] = bvoi[0];
		}
	...
	private String SaveBillLog(String recdata, String rdtdata) throws BusinessException {
		// TODO Auto-generated method stub
		String bill_code = "";
		BillLogVO vo = new BillLogVO();
		SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss:SSS");
		vo.setCreationtime(df.format(new Date()));// 创建时间
		vo.setTransi_type("21"); // 交易类型
		...
		try {
			...
			// TODO: handle exception
			return "生成采购订单日志表保存失败："+e.getMessage();
		}
		return "";
	}
