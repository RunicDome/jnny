...
public class ErContrastUtil {
	...
	public static List<BxcontrastVO> dealContrastForNew(JKBXHeaderVO parentVO, BxcontrastVO[] bxcontrastVOs,
			BXBusItemVO[] items) {
		...
		if ("iweb".equals(parentVO.getSrcsystem()) && "2647".equals(parentVO.getDjlxbm())) {
			contrastVosNew = new ArrayList<BxcontrastVO>();
			for (int i = 0; i < bxcontrastVOs.length; i++) {
				bxcontrastVOs[i].setPk_finitem(items[i].getPrimaryKey());
				contrastVosNew.add(bxcontrastVOs[i]);
			}
		} else if("iweb".equals(parentVO.getSrcsystem())){
			contrastVosNew = new ArrayList<BxcontrastVO>();
		   for (int i = 0; i < bxcontrastVOs.length; i++) {
			   bxcontrastVOs[i].setPk_finitem(items[0].getPrimaryKey());
			   contrastVosNew.add(bxcontrastVOs[i]);
		   }
		}else {
		...
		//comment by zhaoyangm 20180122 单据冲借款之后修改将报销金额改小，不走下面的代码时会出现冲销信息中的fybbje错误
		//但是放开之后，如果是报销单表体多币种，下面的代码计算的本币也是不准确的，暂时不处理
...
//		}
	...
	public static void resetCjkJeToBB(UFDouble bbhl, UFDouble globalhl, UFDouble grouphl, String bzbm, UFDate djrq,
			String zfdwbm, String pk_group, List<BxcontrastVO> contrastVosNew) {
		for (BxcontrastVO cvo : contrastVosNew) {
			try {
				UFDouble cjkbbje = Currency.computeYFB(zfdwbm, Currency.Change_YBJE, bzbm, cvo.getCjkybje(), null,
						null, null, bbhl, djrq)[2];
				cvo.setCjkbbje(cjkbbje);
				UFDouble[] money = Currency.computeGroupGlobalAmount(cvo.getCjkybje(), cjkbbje, bzbm, djrq,
						cvo.getPk_org(), pk_group, globalhl, grouphl);
				cvo.setGroupcjkbbje(money[0]);
				cvo.setGlobalcjkbbje(money[1]);
				UFDouble fybbje = Currency.computeYFB(zfdwbm, Currency.Change_YBJE, bzbm, cvo.getFyybje(), null, null,
						null, bbhl, djrq)[2];
				cvo.setFybbje(fybbje);
				UFDouble[] money2 = Currency.computeGroupGlobalAmount(cvo.getFyybje(), fybbje, bzbm, djrq,
						cvo.getPk_org(), pk_group, globalhl, grouphl);
