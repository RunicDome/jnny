...
public class FYApproveListener implements IBusinessListener {
	...
	public void doAction(IBusinessEvent event) throws BusinessException {
		...
		if (aggvos != null) {
			FeeBalanceHeadVO headvo = (FeeBalanceHeadVO) aggvos[0]
					.getParentVO();
			CircularlyAccessibleValueObject[] childVOs = aggvos[0]
					.getChildrenVO(); // 子表数据
			if (childVOs.length > 0 && !"4D83-01".equals(headvo.getTransi_type()) ) {
				for (int j = 0; j < childVOs.length; j++) {
					FeeBalanceBodyVO bodyvo = (FeeBalanceBodyVO) childVOs[j]; // 子表VO
					if (bodyvo.getPk_wbs() != null && !"".equals(bodyvo)) {
						...
						String sql = "SELECT MIN(KSRQ) FROM (SELECT DISTINCT MX.DEF4 AS KSRQ,MX.PK_FEEBALANCE_B,MIN(MX.MONEY) AS LJWC FROM "
						...
						List<Object[]> wbsls = (List<Object[]>) new BaseDAO()
						...
						if (wbsls != null && wbsls.size() > 0
							...
							String getwbsql = "SELECT PLAN_START_DATE,PLAN_FINISH_DATE FROM PM_WBS WHERE PK_WBS = '"
									+ feeBalanceBody.getPk_wbs() + "'";
							List<Object[]> getls = (List<Object[]>) new BaseDAO()
									.executeQuery(getwbsql,
							...
							if (headvo.getTransi_type().equals("4D83-Cxx-22")
									|| headvo.getTransi_type().equals(
											"4D83-Cxx-04")) {
								jd = Double.parseDouble(feeBalanceBody
										.getDef7() + "");
							} else {
								jd = feeBalanceBody.getMoney().toDouble();
							}
							if(feeBalanceBody.getPk_wbs() != null){
							// XBX新增判断是否已生成，已经生成不再推进度
							IUifService iUifService  = NCLocator.getInstance().lookup(
									IUifService.class);
							WbsVO rwvo = (WbsVO) iUifService
									.queryByPrimaryKey(WbsVO.class, feeBalanceBody.getPk_wbs() + ""); // 项目VO
							if(rwvo != null){
								String rwsql = "SELECT MAX(RWMX.TASKPERCENT) FROM PM_TASKSCHEDULE_B RWMX LEFT JOIN "
										...
										+ "'";
								List<Object[]> rwLs = (List<Object[]>) new BaseDAO()
								.executeQuery(rwsql, new ArrayListProcessor());
								if (rwLs != null && rwLs.size() > 0
										&& rwLs.get(0)[0] != null) {
									String dqjd = rwLs.get(0)[0] + "";
									if (dqjd != null && !"".equals(dqjd)) {
										// 进度不为100 需要推
										if (Double.parseDouble(dqjd) == 100) {
											flag = 0;
										}
									}
								}
							...
							/*if (jd == 100) {
								...
								try {
									if (sdf.parse(ksri).compareTo(
										...
										throw new BusinessException("实际开始日期大于计划完成日期，请修改计划完成日期！");
									}
									String updsql = "UPDATE PM_WBS SET PLAN_START_DATE = '"
											+ ksri
											+ "' WHERE PK_WBS = '"
									...
									if (feeBalanceBody.getDef5() != null
											&& feeBalanceBody.getDef5() != "null"
											&& feeBalanceBody.getDef5() != ""
											&& sdf.parse(
													feeBalanceBody.getDef5())
													.compareTo(
															sdf.parse(wbsstartdate)) == -1) {
										...
										throw new BusinessException("实际完成日期小于计划开始日期，请修改计划开始日期！");
									}
									String upsql = "UPDATE PM_WBS SET PLAN_FINISH_DATE "
											+ "= '"
											+ feeBalanceBody.getDef5()
				...
				/*if ("4D83-Cxx-04".equals(headvo.getTransi_type())) {
					...
					for (int j = 0; j < childrenVOs.length; j++) {
						...
						String def19 = feeBalanceBody.getDef19();
						if (def19 == null || !"Y".equals(def19)) {
							...
							if ("0001A110000000000HYQ".equals(pkOrg) || "0001A21000000000IYIU".equals(pkOrg) ||"0001A21000000000YHPX".equals(pkOrg)) {
								if ((pk_budget != null)
										&& (!"".equals(pk_budget))
										&& (!"null".equals(pk_budget))) { // 如果有项目预算

									// 判断核算要素里面有没有？
									String pk_project = feeBalanceBody
											.getPk_project() + "";
									...
									if (budFactorArrList.size() <= 0) {
										// if ((budFactorOID !=
										// null)&&(!"".equals(budFactorOID))){
										// //如果没有CBS
										// 如果没有 新增一条明细 CBS明细
										insertFactorCbs(pk_budget,
												feeBalanceBody);
										...
										UFDouble budgetfactormny = new UFDouble(
												Double.parseDouble(obj[1] + "")); // 预算明细金额
										UFDouble balafactormny = new UFDouble(
												Double.parseDouble(obj[2] + "")); // 预算余额
										budgetfactormny = budgetfactormny
												.add(feemoney);

										balafactormny = balafactormny
												.add(feemoney);
										System.out.println("budgetfactormny=="
												+ budgetfactormny);
										// 修改核素要素金额
										updateFactorCbs(budgetfactorOID,
												pk_budget, budgetfactormny,
												balafactormny, cbsOID, pkOrg);
									}
									...
									if (budCbsArrList.size() <= 0) {
										// if ((budCbsOID !=
										// null)&&(!"".equals(budCbsOID))){
										// //如果没有CBS
										// 如果没有 新增一条明细 CBS明细
										insertBudgetCbs(pk_budget,
												feeBalanceBody);
								...
								String updsql = "UPDATE PM_FEEBALANCE_B SET DEF19 = 'Y' WHERE PK_FEEBALANCE_B = '"
										+ pk_feebalance_b + "'";
	...
	public void pushTaskProcessByFee(FeeBalanceBillVO billVO)
		...
		for (int i = 0; i < bodyVOArr.length; i++) {
			...
			/*
			 * TaskUtils insertAndUptTaskBIll = NCLocator.getInstance().lookup(
			 * TaskUtils.class);
			 */
			...
			String getwbsql = "SELECT NVL(MAX(MX.TASKPERCENT),0) AS SL FROM PM_TASKSCHEDULE JD LEFT JOIN "
					...
					+ "MX.DR = 0 AND MX.PK_WBS = '" + bodyVO.getPk_wbs() + "'";
