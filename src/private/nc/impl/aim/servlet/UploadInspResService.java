...
import nc.vo.aim.equipinsp.Equipinspb;
...
public class UploadInspResService implements IHttpServletAdaptor {
	...
	public void doAction(HttpServletRequest req, HttpServletResponse resp)
		...
		Debug.error("=============upload EquipInsp msg ：" + body);
		// 上传巡检记录结果
				...
				for (Object inspObj : arr) {
					if (inspObj instanceof JSONObject) {
						...
						SimpleDateFormat df = new SimpleDateFormat(
								"yyyy-MM-dd HH:mm:ss");
						String updSql = "update aim_equipinspb set inspreason = '"
								+ inspJson.getString("inspreason")
								+ "',inspdate = '"
								+ inspJson.getString("inspdate")
								+ "', ts = '"
								+ df.format(new Date())
								+ "' where pk_equipinspb = '"
								+ inspJson.getString("pk_equipinspb") + "';";
	...
	private String checkParam(String body) {
		...
		try {
			...
			for (Object inspObj : arr) {
				if (inspObj instanceof JSONObject) {
					JSONObject inspJson = (JSONObject) inspObj;
					if (inspJson.getString("pk_equipinsp") == null
							|| inspJson.getString("pk_equipinspb") == null
							|| inspJson.getString("inspreason") == null
							|| inspJson.getString("inspdate") == null) {
						return err;
					}
					String pk_equipinspb = inspJson.getString("pk_equipinspb");
					Equipinspb bvo = (Equipinspb) hypubBo.queryByPrimaryKey(
							Equipinspb.class, pk_equipinspb);
					if (bvo == null) {
						return "巡检记录明细主键[" + pk_equipinspb + "]查询失败！";
					}
					if (bvo.getInspreason() != null
							|| bvo.getInspdate() != null) {
						return "巡检记录明细主键[" + pk_equipinspb + "]已完成上报！";
					}
				} else {
					return err;
				}
	...
	private String SaveBillLog(String recdata, String rdtdata)
		...
		vo.setTransi_type("EQUIPINSP"); // 交易类型
