...
public class FeeSaveAction extends SaveAction {
	...
	public void doAction(ActionEvent e) throws Exception {
		...
		FeeBalanceBillVO fee = null;// 获取多编码合同VO
		...
		// String wbs_name = genSaveUtils.getQDWbsName(pk_contracttype);//
		// 对应任务名称
		String ifbl = headVO.getDef4();// 是否补录
		if (ifbl == null || "1001A2100000000B68C3".equals(ifbl)) {
			for (int i = 0; i < bodyVO.length; i++) {
				...
				if (cbsls != null && cbsls.size() > 0) {
					...
					System.out.println("查询开工容缺受理单sql：" + rqsql);
					if (Integer.parseInt(rqls.get(0)[0] + "") <= 0) {
						...
						if (pgdls != null && pgdls.size() > 0) {
							MessageDialog
									.showErrorDlg(null, "提示", "该项目已完成开工单!");
							flag = "false";
							return;
						}
					}
			...
			if ("true".equals(flag)) {
				/*
				 * if (!"".equals(wbs_name)) { String transType =
				 * headVO.getTransi_type();// 交易类型 String pkOrg =
				 * headVO.getPk_org();// 组织 FeeBalanceBillVO newBillVO = new
				 * FeeBalanceBillVO();// 费用结算单聚合VO（修改后） String zbtype =
				 * headVO.getDef1() + "";// 招标结果登记的招标方式 int orgFlag =
				 * genSaveUtils.checkOrg(pkOrg); if (orgFlag == 0) { // 如果没有则跳过
				 * } else if (orgFlag == 1) { 3===================第三步验证WBS任务
				 * FeeBalanceBodyVO[] newBodyVO = genSaveUtils.checkWBS(pkOrg,
				 * transType, bodyVO, zbtype, wbs_name); if (newBodyVO != null
				 * && newBodyVO.length > 0) { newBillVO.setParentVO(headVO);
				 * newBillVO.setChildrenVO(newBodyVO);
				 * this.editor.setValue(newBillVO); } } super.doAction(e); }
				 */
				super.doAction(e);
			}
		}
